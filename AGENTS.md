...
A battery-powered white noise box built around an ESP32-C3 driving a MAX98357A I2S amplifier and a bare EC11 rotary encoder with a push switch. The device generates white, pink, or brown noise and plays it continuously through an attached speaker.
...
- `white_noise.ino` — all firmware. Top-level sections: pin definitions, I2S setup (`initI2S`), noise generators (`generateWhiteSample` / `generatePinkSample` / `generateBrownSample`), audio pump (`processAudio`), rotary-encoder ISR (`encoderISR`, quadrature lookup table), push-button state machine (`loop`), and deep-sleep power management (`enterDeepSleep`, `checkTurnOnCondition`).
...
- **Noise type:** Short press (< 1 s, with debounce) cycles white → pink → brown → white.
